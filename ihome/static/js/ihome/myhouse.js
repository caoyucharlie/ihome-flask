...
$(document).ready(function(){
    ...
    $.get('/house/authmyhouse/', function (data) {
        if(data.code == '200'){
            $(".auth-warn").hide();
            var house_list_html = ''
            for (var i=0; i < data.hlist_list.length; i++){
                var house_html = '<li>'
                house_html += '<a href="/detail.html?id=&f=my">'
                house_html += ' <div class="house-title">'
                house_html += ' <h3>房屋ID:' + data.hlist_list[id].id + '——' + data.hlist_list</h3>'
                house_html += '</div>'
                house_html +=  '<div class="house-content">'
                house_html +=  '<img src="' + data.hlist_list[i].image + '">'
                house_html += '<div class="house-text">'
                house_html +=  '<ul>'
                house_html +=   '<li>位于:' + data.hlist_list[i].area + '">'
                house_html += '<li>价格: ¥' + data.hlist_list_list[i].price + '/晚</li>'
